...
class HONET(nn.Module):
    ...
    def forward(self, x, goals_5, states_total, goals_4, goals_3, goals_2,  mask, step, train_eps, save=True):
        ...
        if ((step % 700) == 0):
            self.hierarchies_selected, hidden_policy_network = self.policy_network(goal_5_vanilla, goal_4_vanilla, goal_3_vanilla, self.hierarchies_selected, self.time_horizon, self.hidden_policy_network, mask, step)
...
class Policy_Network(nn.Module): #z 빼봄
    def __init__(self, d, time_horizon, num_workers):
        super().__init__()
        self.Mrnn = DilatedLSTM(d*3, 3, time_horizon)
        self.num_workers = num_workers
    def forward(self, goal_5_norm, goal_4_norm, goal_3_norm, hierarchies_selected, time_horizon, hidden, mask, step):
        goal_x_info = torch.cat(([goal_5_norm.detach(), goal_4_norm.detach(), goal_3_norm.detach()]), dim=1)
