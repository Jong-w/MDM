...
class HONET(nn.Module):
    def __init__(self,
        ...
        self.device = device
        self.eps = args.eps
        self.hierarchy_ucb_count = torch.ones_like(torch.empty(self.num_workers, )).to(self.device)
        self.hierarchy5_ucb_count = torch.ones_like(torch.empty(self.num_workers, )).to(self.device)
        self.hierarchy4_ucb_count = torch.ones_like(torch.empty(self.num_workers, )).to(self.device)
        self.hierarchy3_ucb_count = torch.ones_like(torch.empty(self.num_workers, )).to(self.device)
        ...
        self.policy_network = Policy_Network(self.hidden_dim[-1],  300, num_workers, device)
    ...
    def forward(self, x, goals_5, states_total, goals_4, goals_3, goals_2,  mask, step, train_eps, save=True):
        ...
        if ((step % 300) == 0):
            self.hierarchies_selected, hidden_policy_network = self.policy_network(z, goal_5_vanilla, goal_4_vanilla, goal_3_vanilla, self.hierarchies_selected, self.time_horizon,
                                                                self.hidden_policy_network, mask, step, self.hierarchy_ucb_count, self.hierarchy5_ucb_count, self.hierarchy4_ucb_count,
                                                                                   self.hierarchy3_ucb_count)
            self.hierarchy_ucb_count += torch.ones_like(torch.empty(self.num_workers, )).to(self.device)
            self.hierarchy5_ucb_count += self.hierarchies_selected[:, 0].to(self.device)
            self.hierarchy4_ucb_count += self.hierarchies_selected[:, 1].to(self.device)
            self.hierarchy3_ucb_count += self.hierarchies_selected[:, 2].to(self.device)
...
class Policy_Network(nn.Module):
    ...
    def forward(self, z, goal_5_norm, goal_4_norm, goal_3_norm, hierarchies_selected, time_horizon, hidden, mask, step,
                hierarchy_ucb_count, hierarchy5_ucb_count, hierarchy4_ucb_count,  hierarchy3_ucb_count):
        ...
        policy_network_result[:, 0] = policy_network_result[:, 0] + 1 * ((torch.log(hierarchy_ucb_count.to(self.device)) / hierarchy5_ucb_count.to(self.device))**(1/2)).reshape(self.num_workers,)
        policy_network_result[:, 1] = policy_network_result[:, 1] + 1 * ((torch.log(hierarchy_ucb_count.to(self.device)) / hierarchy4_ucb_count.to(self.device))**(1/2)).reshape(self.num_workers,)
        policy_network_result[:, 2] = policy_network_result[:, 2] + 1 * ((torch.log(hierarchy_ucb_count.to(self.device)) / hierarchy3_ucb_count.to(self.device))**(1/2)).reshape(self.num_workers,)
