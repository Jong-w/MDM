...
class HONET(nn.Module):
    def __init__(self,
        ...
        self.policy_network = Policy_Network(self.hidden_dim[-1],  self.policy_length, num_workers)
        ...
        self.hidden_percept = init_hidden(args.num_workers, self.time_horizon[0] * self.hidden_dim[-1],
                                    device=device, grad=True)
        self.hidden_policy_network = init_hidden(args.num_workers, self.policy_length * 4 * self.hidden_dim[1],
                                    device=device, grad=True)
...
def mp_loss(storage, next_v_5, next_v_4, next_v_3, next_v_2, next_v_1, args):
    ...
    #hierarchy_drop_reward = Normalizer(hierarchy_drop_reward)
